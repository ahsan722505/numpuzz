...
import {
  ...
  checkWin,
  openFullscreen,
} from "../../helpers/number-riddle/util";
...
const Board = ({ startGame, resetGame, setGameWon, setRecordTime }) => {
  ...
  const clickHandler = (e, pos) => {
    ...
    const { updatedBoard, possible, items, direction } = move(
      pos,
      internalBoard.current,
      styles,
      gameDim
    );
    if (!possible) return;
          ...
          <div key={rowInd} style={{ height: `${100 / gameDim}%` }}>
            {eachRow.map((eachCell, cellInd) => {
              return (
                <div
                  id={eachCell}
                  onClick={(e) => clickHandler(e, [rowInd, cellInd])}
                  onTouchStart={(e) => clickHandler(e, [rowInd, cellInd])}
                  key={cellInd}
                  className={`${styles.cell} ${eachCell !== 0 && styles.bg} ${
                    moveAble.elements[eachCell] === 1 && moveAble.cls
                  } pointer`}
                  style={{ width: `${100 / gameDim}%` }}
                >
                  <span style={{ fontSize: `${15 / gameDim}rem` }}>
                    {eachCell}
                  </span>
                </div>
              );
